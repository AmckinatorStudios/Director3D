...
enum class Property {
    ...
    // --- Ограничения ---
    // Анимируются оба, и в этом их смысл. Сила — плавное включение: камера
    // подхватывает героя за полсекунды, а не прыгает на него. Положение —
    // то, чем «едут» по траектории: без ключей объект стоял бы в её начале.
    ConstraintInfluence, // Constraint.Influence, 0..1
    ConstraintProgress,  // Constraint.Progress, 0..1
