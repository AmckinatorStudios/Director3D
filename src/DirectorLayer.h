...
// ---------------------------------------------------------------------------
// Пакетный рендер из командной строки (`--render`). Инструмент запускается,
// открывает проект, снимает ролик и закрывается сам — без человека за мышью.
//
// Это не отладочный крючок, а рабочий режим: так ролик пересобирается на
// сборочной машине, так он попадает в CI как проверка «полный конвейер жив», и
// так же рендерится очередь из нескольких проектов обычным скриптом оболочки.
// Окно всё равно создаётся: OpenGL без контекста не работает, но под xvfb это
// не требует ни экрана, ни видеокарты.
struct RenderJob {
    bool Active = false;
    ...
    float StartTime = 0.0f, EndTime = 0.0f; // 0 у конца — до конца ролика
    int Samples = 1;                // сглаживание накоплением
    int Quality = 18;               // CRF
};
...
public:
    ...
    explicit DirectorLayer(std::string startupProject = {}, RenderJob job = {});
...
private:
    ...
    // Пакетный рендер: подготовка (после загрузки проекта) и завершение.
    void StartRenderJob();
    void FinishRenderJob();
    // Демонстрационная постановка для --showcase: персонаж, свет, движение
    // камеры с переводом фокуса. Отдельно от BuildDemoAnimation, потому что у
    // той другая задача — быть быстрой проверкой конвейера, а не роликом.
    void BuildShowcase();
    void BuildConstraintDemo();
    ...
    // Пакетный рендер (--render): настраивается в OnAttach, следит за
    // окончанием в OnUpdate и закрывает приложение.
    RenderJob m_job;
