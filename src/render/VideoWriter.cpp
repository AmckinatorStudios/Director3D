...
std::string VideoWriter::BuildCommand(const Settings& s) {
    ...
    // --- Кодирование видео ---
    //
    // Настройки подобраны под то, ЧТО именно выдаёт рендер: почти сплошные
    // плавные градиенты — небо, пол, крупные гладкие поверхности. Это худший
    // случай для 8-битного H.264: на градиенте вместо плавного перехода
    // появляются ступеньки и квадраты блоков, и ролик выглядит «пережатым»
    // даже там, где битрейта хватает с запасом.
    //
    //   -pix_fmt yuv420p     обязателен для совместимости: без него QuickTime и
    //                        часть плееров и браузеров просто не открывают файл;
    //   sws_flags            точная свёртка цветности с округлением и
    //                        дизерингом — иначе переход RGB->YUV сам добавляет
    //                        ступеньки ещё до кодировщика;
    //   -colorspace bt709…   явная разметка. Без неё плеер угадывает матрицу, а
    //                        угадывает он для HD одно, а ffmpeg по умолчанию
    //                        конвертирует по другому — цвета уезжают;
    //   aq-mode=3            распределяет биты в пользу плоских участков, где
    //                        как раз и видно бандинг. Ослаблять деблокинг
    //                        (deblock=-1,-1) здесь НЕЛЬЗЯ, хотя это стандартный
    //                        приём «за резкость»: он бережёт зерно плёнки, а на
    //                        синтетическом градиенте без зерна делает ровно
    //                        обратное — проявляет границы блоков;
    //   -preset slow         при том же CRF даёт заметно чище картинку, а время
    //                        кодирования на фоне времени РЕНДЕРА кадра
    //                        пренебрежимо.
    cmd << " -sws_flags full_chroma_int+accurate_rnd+bitexact"
        << " -c:v libx264 -preset slow -crf " << s.Quality
